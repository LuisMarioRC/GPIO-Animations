...
void setup_keypad()
{
    ...
    gpio_init(LINE1);
    gpio_set_dir(LINE1, GPIO_OUT);

    gpio_init(LINE2);
    gpio_set_dir(LINE2, GPIO_OUT);

    gpio_init(LINE3);
    gpio_set_dir(LINE3, GPIO_OUT);

    gpio_init(LINE4);
    gpio_set_dir(LINE4, GPIO_OUT);
    ...
    while (true)
    {
        ...
        else if (current_key == '3')
        {
            for (int frame = 0; frame < 5; frame++)
            {
                ...
                for (int i = 0; i < LED_COUNT; i++)
                {
                    int intensidade = (i + frame) % 5 == 0 ? 255 : 0;
                    npSetLED(i, intensidade, intensidade / 2, intensidade / 3);
                }
